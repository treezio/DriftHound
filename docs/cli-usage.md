  ...
  drift-check:
    ...
    steps:
      ...
      - name: Install DriftHound CLI
        run: |
          sudo curl -L https://raw.githubusercontent.com/treezio/DriftHound/main/bin/drifthound-cli \
            -o /usr/local/bin/drifthound
          sudo chmod +x /usr/local/bin/drifthound

      - name: Run Drift Check
        env:
          DRIFTHOUND_TOKEN: ${{ secrets.DRIFTHOUND_TOKEN }}
          DRIFTHOUND_URL: ${{ secrets.DRIFTHOUND_URL }}
        run: |
...
drift_check:
  stage: check
  image: ghcr.io/treezio/drifthound:latest
  script:
    - bin/drifthound-cli --tool=terraform
        --project=my-infrastructure
        --environment=production
        --token=$DRIFTHOUND_TOKEN
        --api-url=$DRIFTHOUND_URL
...
pipeline {
    ...
    environment {
        DRIFTHOUND_TOKEN = credentials('drifthound-token')
        DRIFTHOUND_URL = 'https://drifthound.example.com'
    }
            ...
            steps {
                sh '''
                    curl -L https://raw.githubusercontent.com/treezio/DriftHound/main/bin/drifthound-cli \
                        -o drifthound && chmod +x drifthound
